	...
	if [[ $WP_VERSION == 'nightly' || $WP_VERSION == 'trunk' ]]; then
		...
		if [ $WP_VERSION == 'latest' ]; then
			...
			if [[ $WP_VERSION =~ [0-9]+\.[0-9]+\.[0] ]]; then
				...
				# otherwise, scan the releases and get the most up to date minor version of the major version
...
recreate_db() {
	...
	if [[ $1 =~ ^(y|yes)$ ]]
	then
		mysqladmin drop $DB_NAME -f --user="$DB_USER" --password="$DB_PASS" --host="$DB_HOST"
		create_db
...
install_db() {
	...
	# create database
	if [ $(mysql --user="$DB_USER" --password="$DB_PASS"$EXTRA --execute="SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME='$DB_NAME';" | grep -c $DB_NAME) != "0" ]; then
