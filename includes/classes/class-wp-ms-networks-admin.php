...
class WP_MS_Networks_Admin
{
            ...
            if (isset($_GET[$type]) && in_array($_GET[$type], array_keys($success))) :
                ...
                <div id="message" class="<?php echo esc_attr($updated); ?> notice is-dismissible">
                    ...
                    <button type="button" class="notice-dismiss">
                        <span
                            class="screen-reader-text"><?php esc_html_e('Dismiss this notice', 'wp-multi-network'); ?></span>
                    </button>
                </div>
        ...
        <div class="wrap">
            <h1><?php esc_html_e('Networks', 'wp-multi-network');
                ...
                if (current_user_can('create_networks')) : ?>
                    ...
                    <a href="<?php echo esc_url(add_query_arg(array('page' => 'add-new-network'), $this->admin_url())); ?>"
                       class="add-new-h2"><?php echo esc_html_x('Add New', 'network', 'wp-multi-network'); ?></a>
            ...
            <form method="post"
                  action="<?php echo esc_url(add_query_arg(array('action' => 'domains'), $this->admin_url())); ?>"
                  id="domain-search">
            ...
            <form method="post" id="form-domain-list"
                  action="<?php echo esc_url(add_query_arg(array('action' => 'all_networks'), $this->admin_url())); ?>">
                            ...
                            <div id="titlediv">
                                <div id="titlewrap">
                                    <label class="screen-reader-text" id="title-prompt-text"
                                           for="title"><?php echo esc_html_e('Enter network title here', 'wp-multi-network'); ?></label>
                                    <input type="text" name="title" size="30" id="title" spellcheck="true"
                                           autocomplete="off" value="<?php echo esc_attr($network_title); ?>">
                                </div>
                            </div>
                        </div>
    ...
    public function delete_network_page()
    {
        ...
        <div class="wrap">
            ...
            <form method="post" action="<?php echo remove_query_arg('action'); ?>">
                ...
                <a class="button"
                   href="<?php echo esc_url($this->admin_url()); ?>"><?php esc_html_e('Cancel', 'wp-multi-network'); ?></a>
            </form>
        </div>
            ...
            <form method="post" action="<?php echo esc_url($this->admin_url()); ?>">
                <?php if (!empty($sites)) {
                    ...
                    if (RESCUE_ORPHANED_BLOGS) { ?>
                        ...
                        <div class="error">
                            <h3><?php esc_html_e('You have selected the following networks for deletion', 'wp-multi-network'); ?>
                                :</h3>
                            <ul>
                            ...
                            <p><label
                                    for="override"><?php esc_html_e('If you still want to delete these networks, check the following box', 'wp-multi-network'); ?>
                ...
                <input type="submit" name="delete_multiple"
                       value="<?php esc_html_e('Delete Networks', 'wp-multi-network'); ?>" class="button">
                <input type="submit" name="cancel" value="<?php esc_html_e('Cancel', 'wp-multi-network'); ?>"
                       class="button">
            </form>
        </div>
    ...
    private function add_network_handler()
    {
        ...
        if (isSubDomainNetwork()) {
            $network_domain =getFullDomain($network_domain );
        }
    ...
    private function update_network_handler()
    {
        ...
        if (isSubDomainNetwork() && get_main_network_id()!==$network_id) {
            $network_domain = getFullDomain($network_domain);
        }
    ...
    private function reassign_sites_handler()
    {
        ...
        // Find sites which need move into current network
        $movingTo = [];
        ...
        // Find sites which need move out from current network
        $movingFrom = [];
